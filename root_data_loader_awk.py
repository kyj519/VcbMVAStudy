...
- **Weight handling**: the loader **never reads a `weight` leaf**. It **always computes** per-event
  weights from available `weight_*` leaves (and optional RF patch via callback).
...
def _build_weight_vector(arrays: Dict[str, ak.Array], use_btag: bool, include_rf: Optional[np.ndarray]) -> np.ndarray:
    fields = _DEFAULT_WEIGHT_FIELDS_BT if use_btag else _DEFAULT_WEIGHT_FIELDS_CT
...
def _read_uproot(root_path: str, tree_path: str, want_columns: List[str], log_columns: Optional[Sequence[str]] = None, winsorize_columns: Optional[Sequence[Tuple[str, Tuple[float, float]]]] = None) -> Dict[str, ak.Array]:
    if "year_index" in want_columns:
        want_columns.remove("year_index")
        
    with uproot.open(root_path, object_cache=uproot.cache.LRUCache(800_000)) as f:
        t = f[tree_path]
        n = t.num_entries                         
        arrays = t.arrays(list(want_columns), library="ak", how=dict)
    ...
    arrays["__idx"] = ak.Array(np.arange(n, dtype=np.uint64))  # 0..n-1
    return arrays
...
def load_root_as_dataset_kfold(
    ...
    *,
    ...
    weight_cols = set(_DEFAULT_WEIGHT_FIELDS_BT if assume_btag_mode else _DEFAULT_WEIGHT_FIELDS_CT)
    ...
    with task("Preparing to load data..."):
        ...
        for group in tree_path_filter_str:
            ...
            for root_path, tree_path, fstr in sources:
                ...
                want = set(varlist); want.update(weight_cols)
        ...
        with task(f"Loading class {class_idx}"):
            ...
            for (root_path, tree_path, fstr) in sources:
                with task(f"  File: {root_path}, Tree: {tree_path}, Filter: {fstr or '(none)'}"):
                    ...
                    rf_vec = compute_rf_patch(arrays, root_path, rf_patch_provider(assume_btag_mode))
                    w_full = _build_weight_vector(arrays, use_btag=assume_btag_mode, include_rf=rf_vec)
    ...
    for i, f in enumerate(features):
        if np.issubdtype(X_all[f].dtype, np.integer):
            n_unique = np.unique(X_all[f]).size
            if n_unique <= 50:
                categorical_columns.append(f)
                categorical_dims[f] = int(n_unique)
    ##Hardcode year_index as categorical
    categorical_columns = ['year_index'] if 'year_index' in features else categorical_columns
