...
class TabNetTrainConfig(Base):
    ...
    lambda_sparse: float = 1e-3   # Base(1e-4) 오버라이드: 물리 변수 중 노이즈성 피처 가지치기 강화
    
    # --- [2] 학습 파라미터 및 Ghost Batch Norm 세팅 ---
    T0: int = 20
    patience: int = 300
    batch_size: int = 8192 * 4    # 32,768
    num_virtual_minibatches: int = 128 # 32768 / 128 = 가상 배치 사이즈 256 (TabNet 권장치)
    lr: float = 2e-3 
    ...
    varlist: List[str] = field(
        default_factory=lambda: [
            "m_had_w", "pt_w_u", "pt_w_d", "eta_w_u", "eta_w_d",
            "Cat_w_u", "Cat_w_d",
            "logp_class_0", "logp_class_1", "logp_class_2",
            "logp_class_3", "logp_class_4", "logp_class_5",
            "detection_score_logp", "assignment_logp",
            "n_bjets", "n_jets", "n_cjets", "ht"
        ]
    )
    ...
    norm_columns: List[str] = field(default_factory=lambda: [
        "eta_w_u", "eta_w_d",
        "logp_class_0", "logp_class_1", "logp_class_2",
        "logp_class_3", "logp_class_4", "logp_class_5",
        "detection_score_logp", "assignment_logp"
    ...
    log_norm_columns: List[str] = field(default_factory=lambda: [
        "pt_w_u", "pt_w_d", "ht"
    ])

    winsorize_columns: List[Tuple[str, Tuple[float, float]]] = field(default_factory=list)

    winsorize_norm_columns: List[Tuple[str, Tuple[float, float]]] = field(default_factory=lambda: [
        ("m_had_w", (0, 300))
    ])
    ...
    def build_input_tuple(self,
        ...
        for e in eras:
            # 경로 중복을 최소화하여 가독성 개선
            base_mu = f"{sample_folder_loc}/{result_folder_name}/Mu_Unmapped_QuadJet_TemplateTraining_PseudoCont/{e}"
            base_el = f"{sample_folder_loc}/{result_folder_name}/El_Unmapped_QuadJet_TemplateTraining_PseudoCont/{e}"
            ...
            cls0.extend([(*vcb_mu, cuts['RECO_CORRECT_MASK']), (*vcb_el, cuts['RECO_CORRECT_MASK'])])
            cls1.extend([(*vcb_mu, f"!{cuts['RECO_CORRECT_MASK']}"), (*vcb_el, f"!{cuts['RECO_CORRECT_MASK']}")])
            
            cls2.extend([(*ttlj_mu, f"({cuts['HF_BB_MASK']})&&({cuts['CX_DECAY_MASK']})"), 
                         (*ttlj_el, f"({cuts['HF_BB_MASK']})&&({cuts['CX_DECAY_MASK']})")])
                         
            cls3.extend([(*ttlj_mu, f"({cuts['HF_BB_MASK']})&&({cuts['UX_DECAY_MASK']})"), 
                         (*ttlj_el, f"({cuts['HF_BB_MASK']})&&({cuts['UX_DECAY_MASK']})")])
                         
            cls4.extend([(*ttlj_mu, f"({cuts['HF_CC_MASK']})&&({cuts['CX_DECAY_MASK']})"), 
                         (*ttlj_el, f"({cuts['HF_CC_MASK']})&&({cuts['CX_DECAY_MASK']})")])
                         
            cls5.extend([(*ttlj_mu, f"(!{cuts['HF_MASK']})&&({cuts['CX_DECAY_MASK']})"), 
                         (*ttlj_el, f"(!{cuts['HF_MASK']})&&({cuts['CX_DECAY_MASK']})")])
                         
            cls6.extend([(*ttlj_mu, f"(!{cuts['HF_BB_MASK']})&&({cuts['UX_DECAY_MASK']})"), 
                         (*ttlj_el, f"(!{cuts['HF_BB_MASK']})&&({cuts['UX_DECAY_MASK']})")])
